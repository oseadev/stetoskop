...
if ((empty($username))) {
	...
	if (!empty($username) && $password == $confirm_password) {
		$num_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM users WHERE email='" . $username . "'"));

		if ($num_rows == 0) {
			$query = mysqli_query($con, "INSERT INTO users (id, email, password, role_id) VALUES(0,'" . $username . "','" . $password . "', 1)");
